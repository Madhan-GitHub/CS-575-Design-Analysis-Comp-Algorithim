# AIM: IMPLEMENTS THE MINMAX ALGORITHM WITH ALPHA-BETA PRUNING
...
def Maximize(grid, alpha, beta, depth, start):
    ...
    for child in children(grid):
        maxUtility = max(maxUtility, Minimize(
            grid=child, alpha=alpha, beta=beta, depth=depth-1, start=start))
...
def Minimize(grid, alpha, beta, depth, start):
    ...
    for child in children:
        minUtility = min(minUtility, Maximize(
            grid=child,  alpha=alpha, beta=beta, depth=depth-1, start=start))
